...
router.post('/api/disconnect', adminAuth, async (req, res) => {
    ...
    const sanitizedNumber = number.replace(/[^0-9]/g, '');
    if (!activeSockets.has(sanitizedNumber)) return res.status(404).json({ error: 'Session not found' });
    try {
        ...
        socketCreationTime.delete(sanitizedNumber);
        res.json({ success: true, message: `Session ${sanitizedNumber} disconnected` });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/api/delete-session', adminAuth, async (req, res) => {
    ...
    try {
        ...
        const sessionPath = path.join(SESSION_BASE_PATH, `session_${sanitizedNumber}`);
        if (fs.existsSync(sessionPath)) await fs.remove(sessionPath);
        res.json({ success: true, message: `Session ${sanitizedNumber} deleted` });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});

// API: Reconnect session
router.post('/api/reconnect', adminAuth, async (req, res) => {
    const { number } = req.body;
    if (!number) return res.status(400).json({ error: 'Number is required' });
    const sanitizedNumber = number.replace(/[^0-9]/g, '');
    if (activeSockets.has(sanitizedNumber)) return res.json({ success: true, message: `Already connected` });
    try {
        const mockRes = { headersSent: false, send: () => {}, status: () => mockRes, json: () => {} };
...
router.get('/api/settings', adminAuth, async (req, res) => {
    try {
        ...
        res.json({ success: true, settings: { mongodbUri: mongodbUri, adminPin: ADMIN_PIN, defaultConfig: defaultConfig } });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/api/update-mongodb', adminAuth, async (req, res) => {
    ...
    if (!mongodbUri) return res.status(400).json({ error: 'MongoDB URI is required' });
    try {
...
router.post('/api/update-config', adminAuth, async (req, res) => {
    ...
    if (!number || !config) return res.status(400).json({ error: 'Number and config are required' });
    try {
...
router.get('/', async (req, res) => {
    ...
    if (activeSockets.has(sanitizedNumber)) {
        ...
        const uptime = creationTime ? Math.floor((Date.now() - creationTime) / 1000) : 0;
        return res.json({ status: 'already_connected', message: 'Already connected', connectionTime: creationTime ? new Date(creationTime).toLocaleString() : null, uptime: formatUptime(uptime) });
    }
...
router.get('/active', (req, res) => {
    ...
    for (const [number] of activeSockets) {
        const creationTime = socketCreationTime.get(number);
        sessions.push({ number: number, connected: true, since: creationTime ? new Date(creationTime).toLocaleString() : null, uptime: creationTime ? formatUptime(Math.floor((Date.now() - creationTime) / 1000)) : null });
    }
    res.json({ count: activeSockets.size, sessions: sessions });
});
...
router.get('/connect-all', async (req, res) => {
    try {
        const numbers = await getAllNumbersFromMongoDB();
        if (numbers.length === 0) return res.status(404).json({ error: 'No numbers found' });
        const results = [];
        for (const number of numbers) {
            if (activeSockets.has(number)) {
...
process.on('uncaughtException', (err) => {
    ...
    if (process.env.PM2_NAME) exec(`pm2 restart ${process.env.PM2_NAME}`);
});
